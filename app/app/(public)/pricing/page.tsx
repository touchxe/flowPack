    ...
    <div style={{ minHeight: "100vh", background: "#fff" }}>
      <style>{`
        ...
        * { font-family:'Pretendard Variable','Pretendard',-apple-system,BlinkMacSystemFont,system-ui,sans-serif; }
        :root { --brand:#6366F1; --brand2:#8B5CF6; --heading:#111827; --body:#374151; --muted:#9CA3AF; --border:#E5E7EB; --surface:#F9FAFB; }
        .fp-grad { background:linear-gradient(135deg,#6366F1,#8B5CF6); }
        ...
        .plan-card.popular { border-color:#6366F1; box-shadow:0 8px 30px rgba(99,102,241,0.18); }
            ...
            {plans.map((plan) => {
              ...
              return (
                <div key={plan.id} className={`plan-card${isPop ? " popular" : ""}`}
                  ...
                  {/* 인기 뱃지 */}
                  {isPop && (
                    <div style={{ position: "absolute", top: -1, left: "50%", transform: "translateX(-50%)", background: "linear-gradient(135deg,#6366F1,#8B5CF6)", color: "#fff", fontSize: 11, fontWeight: 700, padding: "5px 16px", borderRadius: "0 0 10px 10px", display: "flex", alignItems: "center", gap: 5 }}>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    <div style={{ fontSize: 36, fontWeight: 900, color: isPop ? "#6366F1" : "var(--heading)", lineHeight: 1.1 }}>
                      {fmtPrice(price)}
                      {price !== null && price > 0 && (
                  ...
                  {/* 크레딧 */}
                  <div style={{ padding: "10px 14px", borderRadius: 10, background: isPop ? "#EEF2FF" : "#F9FAFB", marginBottom: 20, fontSize: 13, fontWeight: 700, color: isPop ? "#6366F1" : "#374151", display: "flex", alignItems: "center", gap: 6 }}>
                    <Zap size={14} color={isPop ? "#6366F1" : "#6B7280"} />
                  ...
                  {/* CTA 버튼 */}
                  <button onClick={() => handleSubscribe(plan)} style={{ width: "100%", height: 44, borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: "pointer", border: "none", background: isPop ? "linear-gradient(135deg,#6366F1,#8B5CF6)" : plan.id === "FREE" ? "#F3F4F6" : "#111827", color: isPop ? "#fff" : plan.id === "FREE" ? "#374151" : "#fff", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, transition: "all 0.2s" }}>
      ...
      <section style={{ padding: "64px 24px" }}>
        <div className="mx-auto max-w-3xl" style={{ textAlign: "center" }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            <Link href="/register" style={{ display: "inline-flex", alignItems: "center", gap: 8, height: 52, padding: "0 32px", borderRadius: 12, background: "linear-gradient(135deg,#6366F1,#8B5CF6)", color: "#fff", fontSize: 15, fontWeight: 700, textDecoration: "none", boxShadow: "0 4px 14px rgba(99,102,241,0.35)" }}>
