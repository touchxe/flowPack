...
const PLAN_COLOR: Record<string, { color: string; bg: string }> = {
  ...
  STARTER:    { color: "#818CF8", bg: "#6366F120" },
  PRO:        { color: "#A78BFA", bg: "#8B5CF620" },
      ...
      <div style={{ display: "flex", gap: 4 }}>
        {PLAN_TABS.map(p => (
          <button key={p} onClick={() => { setPlan(p); setPage(1); }}
            style={{ padding: "6px 14px", borderRadius: 8, border: plan === p ? "1px solid #6366F1" : "1px solid transparent", background: plan === p ? "#6366F115" : "none", color: plan === p ? "#818CF8" : "#64748B", fontSize: 12, fontWeight: plan === p ? 700 : 500, cursor: "pointer", transition: "all 0.15s" }}>
            {p}
          </button>
        ))}
      </div>
            ...
            {loading ? (
              ...
              <tr key={user.id} style={{ borderBottom: "1px solid #1E293B50", transition: "background 0.15s" }}
                ...
                <td style={{ padding: "12px 16px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#6366F120", border: "1.5px solid #6366F130", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, color: "#818CF8", flexShrink: 0 }}>
                ...
                <td style={{ padding: "12px 16px" }}>
                  ...
                  <div style={{ width: 80, height: 4, borderRadius: 2, background: "#1E293B", overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${Math.min(100, (user.creditsUsed / Math.max(1, user.creditsTotal)) * 100)}%`, background: "linear-gradient(90deg,#6366F1,#8B5CF6)", borderRadius: 2 }} />
                  </div>
                </td>
                {/* 상태 */}
                <td style={{ padding: "12px 16px" }}>
