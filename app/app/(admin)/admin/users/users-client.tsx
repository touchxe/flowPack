...
const PLAN_COLOR: Record<string, { color: string; bg: string }> = {
  ...
  STARTER:    { color: "var(--brand-500)", bg: "rgba(var(--brand-rgb), 0.13)" },
  PRO:        { color: "var(--brand-500)", bg: "rgba(6, 182, 212, 0.13)" },
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(var(--brand-rgb), 0.13)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Users size={17} color="var(--brand-500)" />
          </div>
          <div>
      ...
      <div style={{ display: "flex", gap: 4 }}>
        {PLAN_TABS.map(p => (
          <button key={p} onClick={() => { setPlan(p); setPage(1); }}
            style={{ padding: "6px 14px", borderRadius: 8, border: plan === p ? "1px solid var(--brand-500)" : "1px solid transparent", background: plan === p ? "rgba(var(--brand-rgb), 0.08)" : "none", color: plan === p ? "var(--brand-500)" : "#64748B", fontSize: 12, fontWeight: plan === p ? 700 : 500, cursor: "pointer", transition: "all 0.15s" }}>
            {p}
          </button>
        ))}
      </div>
            ...
            {loading ? (
              ...
              <tr key={user.id} style={{ borderBottom: "1px solid #1E293B50", transition: "background 0.15s" }}
                ...
                <td style={{ padding: "12px 16px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: "rgba(var(--brand-rgb), 0.13)", border: "1.5px solid rgba(var(--brand-rgb), 0.19)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, color: "var(--brand-500)", flexShrink: 0 }}>
