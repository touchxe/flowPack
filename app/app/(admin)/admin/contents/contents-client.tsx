...
const TYPE_CFG: Record<string, { label: string; color: string; bg: string }> = {
  CAROUSEL:    { label: "카드뉴스", color: "var(--brand-500)", bg: "rgba(var(--brand-rgb), 0.13)" },
  BLOG:        { label: "블로그",   color: "#34D399", bg: "#10b98120" },
  VIDEO:       { label: "영상",     color: "#FCD34D", bg: "#F59E0B20" },
  BULK:        { label: "대량",     color: "#F87171", bg: "#ef444420" },
  URL_TO_POST: { label: "URL변환",  color: "var(--brand-500)", bg: "rgba(6, 182, 212, 0.13)" },
};
...
function AILogPanel({ detail, onClose }: { detail: ContentDetail; onClose: () => void }) {
  ...
  const SectionTitle = ({ icon: Icon, label }: { icon: React.ElementType; label: string }) => (
    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, marginTop: 16 }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 700, color: "var(--brand-500)", textTransform: "uppercase" as const, letterSpacing: "0.06em" }}>{label}</span>
    </div>
  );
        ...
        <div style={{ padding: "16px 20px", borderBottom: "1px solid #1E293B", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: "rgba(var(--brand-rgb), 0.08)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Bot size={15} color="var(--brand-500)" />
            </div>
            <div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "0 20px 24px" }}>
          ...
          <div style={{ background: "#0F172A", border: "1px solid #1E293B", borderRadius: 10, padding: "12px 14px", display: "flex", gap: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <p style={{ fontSize: 13, fontWeight: 600, color: detail.aiModel ? "var(--brand-500)" : "#475569", margin: "2px 0 0" }}>
                {detail.aiModel || "미기록"}
              </p>
            </div>
            {timestamp && (
            ...
            <>
              ...
              <div style={{ background: "#0F172A", border: "1px solid #1E293B", borderRadius: 10, padding: "10px 14px" }}>
                <a href={sourceUrl} target="_blank" rel="noopener noreferrer"
                  style={{ fontSize: 12, color: "var(--brand-500)", wordBreak: "break-all" }}>{sourceUrl}</a>
              </div>
            </>
          )}
            ...
            <div key={i} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                <span style={{
                  ...
                  color: msg.role === "system" ? "var(--brand-500)" : "#34D399",
      ...
      <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 4 }}>
        {TYPE_TABS.map(t => (
          <button key={t} onClick={() => { setType(t); setPage(1); }}
            style={{ padding: "6px 12px", borderRadius: 8, border: type === t ? "1px solid var(--brand-500)" : "1px solid transparent", background: type === t ? "rgba(var(--brand-rgb), 0.08)" : "none", color: type === t ? "var(--brand-500)" : "#64748B", fontSize: 12, fontWeight: type === t ? 700 : 500, cursor: "pointer", transition: "all 0.15s" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 4 }}>
        {STATUS_TABS.map(s => (
          <button key={s} onClick={() => { setStatus(s); setPage(1); }}
            style={{ padding: "6px 12px", borderRadius: 8, border: status === s ? "1px solid var(--brand-500)" : "1px solid transparent", background: status === s ? "rgba(var(--brand-rgb), 0.08)" : "none", color: status === s ? "var(--brand-500)" : "#64748B", fontSize: 12, fontWeight: status === s ? 700 : 500, cursor: "pointer", transition: "all 0.15s" }}>
                ...
                <tr key={c.id} style={{ borderBottom: "1px solid #1E293B50", transition: "background 0.15s", cursor: "pointer" }}
                  ...
                  <td style={{ padding: "12px 16px" }}>
                    <button onClick={(e) => { e.stopPropagation(); openDetail(c.id); }}
                      style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 8px", borderRadius: 6, border: "1px solid rgba(var(--brand-rgb), 0.19)", background: "none", color: "var(--brand-500)", fontSize: 10, fontWeight: 600, cursor: "pointer" }}>
                      <Eye size={10} /> 로그
                    </button>
                  </td>
