...
const TYPE_CFG: Record<string, { label: string; color: string; bg: string }> = {
  ...
  URL_TO_POST: { label: "URL변환",  color: "#A78BFA", bg: "#8B5CF620" },
};
      ...
      <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 4 }}>
        {TYPE_TABS.map(t => (
          <button key={t} onClick={() => { setType(t); setPage(1); }}
            style={{ padding: "6px 12px", borderRadius: 8, border: type === t ? "1px solid #6366F1" : "1px solid transparent", background: type === t ? "#6366F115" : "none", color: type === t ? "#818CF8" : "#64748B", fontSize: 12, fontWeight: type === t ? 700 : 500, cursor: "pointer", transition: "all 0.15s" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 4 }}>
        {STATUS_TABS.map(s => (
          <button key={s} onClick={() => { setStatus(s); setPage(1); }}
            style={{ padding: "6px 12px", borderRadius: 8, border: status === s ? "1px solid #6366F1" : "1px solid transparent", background: status === s ? "#6366F115" : "none", color: status === s ? "#818CF8" : "#64748B", fontSize: 12, fontWeight: status === s ? 700 : 500, cursor: "pointer", transition: "all 0.15s" }}>
