...
import { Brain, Zap, DollarSign, AlertTriangle, Activity } from "lucide-react";
...
const MODEL_COLORS: Record<string, string> = {
  "gpt-4o": "#6366F1", "gpt-4o-mini": "#10b981",
  "gpt-4-turbo": "#F59E0B", "gpt-3.5-turbo": "#64748b",
};
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900 p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className={`flex h-8 w-8 items-center justify-center rounded-lg ${accent.replace("text-", "bg-").replace(/\d00/, "500/15")}`}>
...
const CHART_STYLE = "rounded-xl border border-slate-800 bg-slate-900 p-5";
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">OpenAI API 호출 현황 및 비용 모니터링</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <KpiCard label="추정 비용" value={`$${kpi.totalCost.toFixed(2)}`} sub="이번 달 OpenAI 비용" icon={DollarSign} accent="text-amber-400" />
        ...
        <div className={`lg:col-span-2 ${CHART_STYLE}`}>
          ...
          {charts.featureChart.length > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={charts.featureChart} barSize={28}>
                ...
                />
                <Bar dataKey="tokens" name="토큰" radius={[4, 4, 0, 0]}>
            ...
            <div className="flex items-center justify-center h-[200px] text-slate-600 text-sm">데이터 없음</div>
          )}
        </div>

        <div className={CHART_STYLE}>
          <p className="text-sm font-semibold text-slate-300 mb-4">모델별 비용 분포</p>
          ...
          {topUsers.length === 0 ? (
            ...
            <table className="w-full text-xs">
              <thead><tr className="border-b border-slate-800">
                <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-600">유저</th>
                <th className="px-4 py-2 text-right text-[10px] font-bold text-slate-600">호출</th>
                <th className="px-4 py-2 text-right text-[10px] font-bold text-slate-600">토큰</th>
                <th className="px-4 py-2 text-right text-[10px] font-bold text-slate-600">비용</th>
              </tr></thead>
              <tbody>
                {topUsers.map((u, i) => (
                  <tr key={u.email} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                    ...
                    <td className="px-4 py-2 text-right text-slate-400">{u.calls}</td>
                    <td className="px-4 py-2 text-right text-slate-300 font-mono">{u.tokens.toLocaleString()}</td>
                    <td className="px-4 py-2 text-right text-amber-400 font-semibold">${u.cost.toFixed(3)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="max-h-[300px] overflow-y-auto">
              <table className="w-full text-xs">
                <thead><tr className="border-b border-slate-800 sticky top-0 bg-slate-900">
                  <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-600">시간</th>
                  <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-600">유저</th>
                  <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-600">기능</th>
                  <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-600">코드</th>
                </tr></thead>
                <tbody>
                  {errorLogs.map(e => (
