...
export default function ContentViewPage() {
  ...
  if (error || !content) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", flexDirection: "column", gap: 12 }}>
      ...
      <Link href="/contents" style={{ fontSize: 13, color: "var(--fp-primary-subtle0)" }}>← 목록으로</Link>
    </div>
  );
    ...
    <div style={{ minHeight: "100%", background: "#F7F8FA" }}>
      <style>{`
        ...
        * { font-family:'Pretendard Variable','Pretendard',-apple-system,sans-serif; }
        ...
        .view-back:hover { color:var(--fp-primary-subtle0); }
        .edit-link { display:inline-flex; align-items:center; gap:6px; height:36px; padding:0 16px; border-radius:9px; font-size:13px; font-weight:700; background:linear-gradient(135deg,var(--fp-primary-subtle0),var(--fp-primary-subtle0)); color:#fff; text-decoration:none; box-shadow:0 2px 8px rgba(99,102,241,0.3); transition:all 0.2s; }
        ...
        .deploy-btn:hover { border-color:#C7D2FE; color:var(--fp-primary-subtle0); }
        ...
        .slide-nav:hover:not(:disabled) { border-color:#C7D2FE; color:var(--fp-primary-subtle0); }
        ...
        .pub-btn:hover { border-color:#C7D2FE; color:var(--fp-primary-subtle0); }
        ...
        .content-nav-btn:hover { border-color:#C7D2FE; color:var(--fp-primary-subtle0); background:#F8F7FF; }
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #F3F4F6", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 8px", borderRadius: 6, background: "#EEF2FF", color: "var(--fp-primary-subtle0)" }}>
            {isBlog
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button className="pub-btn" onClick={loadPublishes}>
            <Globe size={13} /> 배포 기록
            {publishes.length > 0 && (
              <span style={{ background: "var(--fp-primary-subtle0)", color: "#fff", fontSize: 10, fontWeight: 800, padding: "1px 6px", borderRadius: 9999 }}>
                {publishes.length}
              </span>
            )}
          </button>
      ...
      {showPublishes && (
        <div style={{ background: "#fff", borderBottom: "1px solid #F3F4F6", padding: "16px 24px" }}>
          <h3 style={{ fontSize: 13, fontWeight: 700, color: "#374151", marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
            <Globe size={13} color="var(--fp-primary-subtle0)" /> SNS 배포 기록
          </h3>
          {loadingPublishes ? (
                ...
                <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderRadius: 12, border: "1.5px solid #E5E7EB", background: p.status === "SUCCESS" ? "#F0FDF4" : p.status === "FAILED" ? "#FEF2F2" : "#F9FAFB" }}>
                  ...
                  {p.status === "SUCCESS" && (
                    <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 8, background: "#EEF2FF", flexShrink: 0 }}>
                      <span style={{ fontSize: 16, fontWeight: 800, color: "var(--fp-primary-subtle0)" }}>{p.clickCount ?? 0}</span>
                      <span style={{ fontSize: 10, fontWeight: 600, color: "var(--fp-primary-subtle0)" }}>클릭</span>
                    </div>
                  )}
                  ...
                  {p.trackingUrl && p.status === "SUCCESS" && (
                    <button
                      ...
                      style={{ padding: "5px 10px", borderRadius: 7, border: "1px solid #E5E7EB", background: "#fff", cursor: "pointer", fontSize: 11, fontWeight: 600, color: "var(--fp-primary-subtle0)", display: "flex", alignItems: "center", gap: 4, flexShrink: 0, fontFamily: "inherit" }}>
                      <Link2 size={11} /> 추적 URL
                    </button>
                  )}

                  {p.platformPostUrl && (
                    <a href={p.platformPostUrl} target="_blank" rel="noopener noreferrer" style={{ color: "var(--fp-primary-subtle0)", display: "flex", alignItems: "center", flexShrink: 0 }}>
        ...
        {isBlog ? (
          ...
          <div>
            ...
            {slides.length === 0 ? (
              ...
              <>
                <div style={{ background: "linear-gradient(135deg,var(--fp-primary-subtle0),var(--fp-primary-subtle0))", borderRadius: 20, padding: "48px 40px", color: "#fff", minHeight: 320, display: "flex", flexDirection: "column", justifyContent: "center", textAlign: "center", boxShadow: "0 20px 60px rgba(99,102,241,0.35)", position: "relative" }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {slides.map((_, i) => (
                      <button key={i} className="slide-dot"
                        style={{ background: i === activeSlide ? "var(--fp-primary-subtle0)" : "#E5E7EB", transform: i === activeSlide ? "scale(1.3)" : "scale(1)" }}
                    ...
                    {slides.map((slide, i) => (
                      <div key={i} onClick={() => setActiveSlide(i)}
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <span style={{ fontSize: 11, fontWeight: 800, color: "var(--fp-primary-subtle0)", background: "#EEF2FF", padding: "2px 8px", borderRadius: 5, flexShrink: 0 }}>{i + 1}</span>
                          <div>
