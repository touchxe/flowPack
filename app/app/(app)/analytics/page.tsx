          ...
          {loading ? <div className="skeleton" style={{ height: 200 }} /> : (
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={chartData}>
                ...
                <Area type="monotone" dataKey="count" name="생성" stroke="#6366F1" strokeWidth={2.5} fill="url(#areaGrad)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
          ...
          {loading ? <div className="skeleton" style={{ height: 200 }} /> : (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={platformStats}>
                ...
                <YAxis fontSize={11} tickLine={false} axisLine={false} tick={{ fill: "#9CA3AF" }} />
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="views" name="조회수" radius={[6, 6, 0, 0]} fill="#6366F1" />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
      ...
      <div className="chart-card" style={{ marginTop: 16 }}>
        ...
        {funnel && (
          <div style={{ display: "flex", alignItems: "center", gap: 0, marginBottom: 20 }}>
            {[
              { label: "발행", value: funnel.created, color: "#6366F1", bg: "#EEF2FF", pct: "100%" },
              { label: "배포", value: funnel.distributed, color: "#8B5CF6", bg: "#F5F3FF", pct: funnel.created > 0 ? `${Math.round((funnel.distributed / funnel.created) * 100)}%` : "0%" },
                    ...
                    <td style={{ padding: "10px 12px", textAlign: "right" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8 }}>
                        <div style={{ width: 60, height: 6, borderRadius: 3, background: "#F3F4F6", overflow: "hidden" }}>
                          <div style={{ width: `${barWidth}%`, height: "100%", borderRadius: 3, background: "linear-gradient(90deg, #6366F1, #8B5CF6)" }} />
                        </div>
